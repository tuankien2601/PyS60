    ...
    if (handlernum >= 0) {
        ...
        if (v == Py_None) {
            ...
            if (handlernum == CharacterData && self->in_callback)
                /* On Symbian, the armv5 compiler throws an error
                   stating that the assignment of type 'void(*)' to type
                   'xmlhandler' cannot be done. Explicitly casting the 
                   function to (void *) solves the issue. Adding (void *) 
                   doesn't break the behavior on other platforms so 
                   platform check is not needed. */  
                c_handler = (void *) noop_character_data_handler;
            v = NULL;
        }
        else if (v != NULL) {
            Py_INCREF(v);
