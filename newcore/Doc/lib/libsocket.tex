...
of dictionaries in the format \code{[{\{}'iapid': \var{ap_id}, 
'name': \var{ap_name}{\}}, ...]} where \var{ap_id} will contain the unique
access point ID and \var{ap_name} will have the corresponding name associated
with that ID.
...
\begin{funcdesc}{set_default_access_point}{access_point_name}
...
further socket related function calls (\code{connect}, \code{send},
\code{write} etc) and all the host resolver function calls (\code{getaddrinfo},
\code{getnameinfo}, \code{gethostbyname}, \code{getaddrbyname} etc). To remove
the default interface, pass \code{None} as the argument.
Availabilty: Symbian/S60
