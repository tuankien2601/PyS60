...
class urlretrieve_FileTests(unittest.TestCase):
    ...
    def test_basic(self):
        ...
        if sys.platform == 'symbian_s60':
            # Reasons for changing this assertEqual check :
            # - As TESTFN is a path the value result[0] will have
            # 4 back slashes(c:\\\\testfile) but TESTFN has only 2
            # - Even though the above problem can be circumvented by
            # doing a repr() on TESTFN, the drive name returned is 'C'
            # instead of 'c' and assert fails eventually
            (drive, direc) = os.path.splitdrive(test_support.TESTFN)
            drive = drive.upper()
            direc = '\\' + direc
            temppath = os.path.join(drive, direc)
            self.assertEqual(result[0], temppath)
        else:
